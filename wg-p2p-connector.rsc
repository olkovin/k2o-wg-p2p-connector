...
:if ($myRole = "server") do={
    ...
    # Reset received client pubkey from previous runs
    :set p2pReceivedClientPubkey
}
...
:if ($myRole = "server") do={
    ...
    # Check if SSTP port is available
    :local sstpPortInUse false
    :do {
        :local existingSstp [/interface sstp-server server get enabled]
        :if ($existingSstp) do={
            :local existingPort [/interface sstp-server server get port]
            :if ($existingPort = $p2pSstpPort) do={
                :set sstpPortInUse true
                :put ("   WARNING: SSTP server already running on port " . $p2pSstpPort)
            }
        }
    } on-error={}

    # Add temporary firewall rule for SSTP (will be removed after key exchange)
    :local sstpFwRuleName ("p2p-k2o-sstp-temp-" . $tunnelID)
    :local dropRulesSstp [/ip firewall filter find where chain="input" action~"drop|reject" !disabled]
    :if ([:len $dropRulesSstp] > 0) do={
        :local firstDropSstp [:pick $dropRulesSstp 0]
        /ip firewall filter add chain=input protocol=tcp dst-port=$p2pSstpPort action=accept comment=$sstpFwRuleName place-before=$firstDropSstp
        :put ("   Firewall rule for SSTP added (port " . $p2pSstpPort . ")")
    } else={
        /ip firewall filter add chain=input protocol=tcp dst-port=$p2pSstpPort action=accept comment=$sstpFwRuleName
        :put ("   Firewall rule for SSTP added (port " . $p2pSstpPort . ")")
    }

    # Create temporary PPP profile (remove old profiles first)
    :local profileName ("p2p-k2o-profile-" . $tunnelID)
    :do { /ppp profile remove [find where name~"p2p-k2o-profile"] } on-error={}
    ...
    # Create PPP secret (remove existing if any)
    :local secretName ("p2p-k2o-" . $tunnelID)
    :do { /ppp secret remove [find where name=$p2pSstpUser] } on-error={}
    ...
    :set cancelSource ($cancelSource . ":do { /ip firewall filter remove [find where comment~\"p2p-k2o-sstp-temp\"] } on-error={}\r\n")
    :set cancelSource ($cancelSource . ":put \"Adoption cancelled (SSTP and firewall cleaned up)\"\r\n")
    ...
    :do { /system script remove [find where name="p2p-k2o-cancel"] } on-error={}
    :do { /system script remove [find where name="p2p-k2o-finalize"] } on-error={}
    # Remove temporary SSTP firewall rule
    :do { /ip firewall filter remove [find where comment~("p2p-k2o-sstp-temp-" . $tunnelID)] } on-error={}
    :put "   SSTP cleaned up (including firewall rule)"
    ...
    :if (!$connected) do={
        ...
        # Remove existing finalize script if any
        :do { /system script remove [find where name="p2p-k2o-finalize"] } on-error={}
...
:set removeSource ($removeSource . "# Safety: Run 3 times within 30 seconds to confirm removal\r\n")
...
:set removeSource ($removeSource . ":global p2pRemoveConfirm" . $tunnelID . "\r\n")
:set removeSource ($removeSource . ":global p2pRemoveTime" . $tunnelID . "\r\n")
:set removeSource ($removeSource . "\r\n")
:set removeSource ($removeSource . ":local now [/system clock get time]\r\n")
:set removeSource ($removeSource . ":local nowSec (([:pick \$now 0 2] * 3600) + ([:pick \$now 3 5] * 60) + [:pick \$now 6 8])\r\n")
:set removeSource ($removeSource . "\r\n")
:set removeSource ($removeSource . "# Check if previous confirmation expired (30 sec)\r\n")
:set removeSource ($removeSource . ":if ([:typeof \$p2pRemoveTime" . $tunnelID . "] != \"nil\") do={\r\n")
:set removeSource ($removeSource . "    :local elapsed (\$nowSec - \$p2pRemoveTime" . $tunnelID . ")\r\n")
:set removeSource ($removeSource . "    :if (\$elapsed < 0) do={ :set elapsed (\$elapsed + 86400) }\r\n")
:set removeSource ($removeSource . "    :if (\$elapsed > 30) do={\r\n")
:set removeSource ($removeSource . "        :set p2pRemoveConfirm" . $tunnelID . " 0\r\n")
:set removeSource ($removeSource . "    }\r\n")
:set removeSource ($removeSource . "}\r\n")
:set removeSource ($removeSource . "\r\n")
:set removeSource ($removeSource . ":if ([:typeof \$p2pRemoveConfirm" . $tunnelID . "] = \"nil\") do={ :set p2pRemoveConfirm" . $tunnelID . " 0 }\r\n")
:set removeSource ($removeSource . ":set p2pRemoveConfirm" . $tunnelID . " (\$p2pRemoveConfirm" . $tunnelID . " + 1)\r\n")
:set removeSource ($removeSource . ":set p2pRemoveTime" . $tunnelID . " \$nowSec\r\n")
:set removeSource ($removeSource . "\r\n")
:set removeSource ($removeSource . ":if (\$p2pRemoveConfirm" . $tunnelID . " < 3) do={\r\n")
:set removeSource ($removeSource . "    :local remaining (3 - \$p2pRemoveConfirm" . $tunnelID . ")\r\n")
:set removeSource ($removeSource . "    :put \"==================================================\"\r\n")
:set removeSource ($removeSource . "    :put \"  REMOVAL CONFIRMATION REQUIRED\"\r\n")
:set removeSource ($removeSource . "    :put \"==================================================\"\r\n")
:set removeSource ($removeSource . "    :put (\"  Tunnel ID: \" . \$tid)\r\n")
:set removeSource ($removeSource . "    :put \"\"\r\n")
:set removeSource ($removeSource . "    :put (\"  Run this script \" . \$remaining . \" more time(s)\")\r\n")
:set removeSource ($removeSource . "    :put \"  within 30 seconds to confirm removal.\"\r\n")
:set removeSource ($removeSource . "    :put \"==================================================\"\r\n")
:set removeSource ($removeSource . "    :error \"Confirmation required\"\r\n")
:set removeSource ($removeSource . "}\r\n")
:set removeSource ($removeSource . "\r\n")
:set removeSource ($removeSource . "# Reset confirmation\r\n")
:set removeSource ($removeSource . ":set p2pRemoveConfirm" . $tunnelID . "\r\n")
:set removeSource ($removeSource . ":set p2pRemoveTime" . $tunnelID . "\r\n")
:set removeSource ($removeSource . "\r\n")
