...
# Create WireGuard interface
:if ($myRole = "server") do={
    /interface wireguard add name=$wgInterfaceName listen-port=$p2pWgPort mtu=1420 comment=$p2pComment
} else={
...
:if ($myRole = "server") do={
    ...
    :if ([:len $dropRules] > 0) do={
        ...
        /ip firewall filter add chain=input protocol=udp dst-port=$p2pWgPort action=accept comment=$p2pComment place-before=$firstDrop
        :put "   Firewall rule added (before first drop)"
    } else={
        /ip firewall filter add chain=input protocol=udp dst-port=$p2pWgPort action=accept comment=$p2pComment
    ...
    /interface sstp-client add name=$sstpClientName connect-to=($peerAddress . ":" . $p2pSstpPort) user=$p2pSstpUser password=$p2pSstpPass profile=default-encryption authentication=mschap2 verify-server-certificate=no verify-server-address-from-certificate=no comment=$p2pComment
    ...
    :do {
        :local sshCmd (":global p2pClientPubkeyInput \"" . $myPubkey . "\"; /system script run p2p-k2o-register-" . $tunnelID)
        # Try different registration script names (server might have different ID)
        :local regScripts [/system script find where name~"p2p-k2o-register"]

        # Execute via SSH
        /system ssh-exec address=10.254.254.1 user=$p2pSstpUser command=$sshCmd output-to-file=("p2p-ssh-result-" . $tunnelID)
        :delay 2s

        :local resultFile [/file find where name=("p2p-ssh-result-" . $tunnelID . ".txt")]
    ...
    :if ([:len $serverPubkey] < 40) do={
        ...
        :put ""
        :put "  1. Copy this key to SERVER"
        :put "  2. Get server's public key and tunnel ID"
        :put "  3. Run on THIS router:"
        :put "     :global p2pServerPubkey \"SERVER_KEY_HERE\""
        :put "     :global p2pServerTunnelID \"SERVER_TUNNEL_ID\""
        :put "     /system script run p2p-k2o-finalize"
    ...
    :put "[5/6] Adding WireGuard peer..."
    :local peerName ("wg-p2p-k2o-peer-" . $tunnelID)
    /interface wireguard peers add interface=$wgInterfaceName name=$peerName public-key=$serverPubkey endpoint-address=$peerAddress endpoint-port=$p2pWgPort allowed-address=($peerWgIP . "/32") persistent-keepalive=25s comment=$p2pComment
    :put ("   Peer added: " . $peerName)
